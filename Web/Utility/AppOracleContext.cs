...
namespace Web.Utility
{
    ...
    public class AppOracleContext : IAppContext
    {
        public IEnumerable<T> GetPaged<T>(
            string table,
            string orderBy,
            out int total,
            string fields = "*",
            ...
            object param = null)
        {
            ...
            using (var cn = this.ConnectionFactory.CreateConnection())
            {
                ...
                if (string.IsNullOrEmpty(orderBy))
                {
                    ...
                    if (
                        cn.Query<int>(
                            string.Format("select count(*) from user_tables where table_name='{0}'", tempTable))
                            .FirstOrDefault() == 0)
                    {
                        throw new Exception("查询的表不存在");
                    }

                    orderBy = cn.Query<string>(string.Format(@"select cu.column_name 
from user_cons_columns cu, 
user_constraints au where cu.constraint_name = au.constraint_name and
 au.constraint_type = 'P' and au.table_name = '{0}'", tempTable)).FirstOrDefault();
                    if (string.IsNullOrEmpty(orderBy))
                    {
        ...
        public IConnectionFactory ConnectionFactory
        {
            get
            {
                return InstanceLocator.Current.GetInstance<IConnectionFactory>("OracleConnectionFactory");
            }
        }
